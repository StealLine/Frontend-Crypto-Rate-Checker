...
```
Serves files from `/usr/share/nginx/html`.Proxies all req to http://cryptovault:8080; where cryptovault host of the container where this app is working
...
### What it does
Handles user authentication. Does not connect to PostgreSQL directly — all SQL is sent to [PROXY_TODB](https://github.com/StealLine/PROXY_DB_APP) which executes it in the correct database.
...
where PROXYAPI__BASEURL is env variable which points to hostname where [PROXY_TODB](https://github.com/StealLine/PROXY_DB_APP) works. Ex PROXYAPI__BASEURL: http://proxy_dep:8080

*Please note this applications must be running in same network either just locally or same network in docker*
...
```
Default address of [PROXY_TODB](https://github.com/StealLine/PROXY_DB_APP) . Overridden in Docker by environment variable `PROXYAPI__BASEURL`.
...
### `AuthService.cs`
Contains all the actual logic. For every operation it builds a SQL string and sends it to [PROXY_TODB](https://github.com/StealLine/PROXY_DB_APP) via `POST /Home/ExecuteScript` with the hash from `AppState.DbHash`.
...
## How to run locally?
1. Build nginx image using dockerfile_nginx
2. Build project locally by running dotnet publish -c Release -o publish
3. Make sure dockerfile_app file in the same folder as publish/ folder generated by above command
4. Build application image using dockerfile_app file
5. Run it with the docker-compose.yml make sure there is [PROXY_TODB](https://github.com/StealLine/PROXY_DB_APP) working in the same network, also make sure to add volume to db_hash file (it is located by path app/secret_data/db_hash.txt in the docker container)
6. Also dont forget to add PROXYAPI__BASEURL environment with the hostname of [PROXY_TODB](https://github.com/StealLine/PROXY_DB_APP).
